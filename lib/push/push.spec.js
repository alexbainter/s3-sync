...
describe('push', () => {
  ...
  it('should treat pages specified with pageNames as HTML files', () => {
    const distDir = './dist/';
    const pageNames = ['page1', 'more-pages/page2'];
    mockFs(
      pageNames.reduce((o, filename) => {
        o[`${distDir}${filename}`] = getContentForFilename(filename);
        return o;
      }, {})
    );
    ...
    return push({
      ...
      expect(mockS3.upload.callCount).to.equal(pageNames.length);
      const allUploadParams = mockS3.upload
        .getCalls()
        .map(({ firstArg }) => firstArg);
      pageNames.forEach((pageName) => {
        ...
        expect(uploadParams).to.exist;
        expect(uploadParams).to.have.property('ContentType', 'text/html');
      });
    });
  });
});
